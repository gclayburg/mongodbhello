...
package com.garyclayburg.attributes.groovyfork;
...
public class ForkedGroovyScriptEngine implements ResourceConnector {

    @SuppressWarnings("UnusedDeclaration")
    private static final Logger log = LoggerFactory.getLogger(ForkedGroovyScriptEngine.class);
                ...
                public LookupResult findClassNode(String origName, CompilationUnit compilationUnit) {
                    ...
                    for (String ext : cc.getScriptExtensions()) {
                        try {
                            ...
                            if (ForkedGroovyScriptEngine.this.isSourceNewer(entry)) {
                                try {
        ...
        private Class doParseClass(GroovyCodeSource codeSource) {
            ...
            if (origDep != null) {
                ...
                for (String depName : origDep) {
                    ...
                    try{
                        if (origEntry==dep || ForkedGroovyScriptEngine.this.isSourceNewer(dep)) {
                            newDep.add(depName);
                        }
                    } catch (ResourceException re) {
    ...
    public static void main(String[] urls) throws Exception {
        ForkedGroovyScriptEngine gse = new ForkedGroovyScriptEngine(urls);
     ...
     */
    private ForkedGroovyScriptEngine(URL[] roots, ClassLoader parent, ResourceConnector rc) {
    ...
    public ForkedGroovyScriptEngine(URL[] roots) {
        this(roots, CL_STUB, null);
    }

    public ForkedGroovyScriptEngine(URL[] roots, ClassLoader parentClassLoader) {
        this(roots, parentClassLoader, null);
    }

    public ForkedGroovyScriptEngine(String[] urls) throws IOException {
    ...
    public ForkedGroovyScriptEngine(String[] urls, ClassLoader parentClassLoader) throws IOException {
        this(createRoots(urls), parentClassLoader, null);
    }

    public ForkedGroovyScriptEngine(String url) throws IOException {
        this(new String[]{url});
    }

    public ForkedGroovyScriptEngine(String url, ClassLoader parentClassLoader) throws IOException {
        this(new String[]{url}, parentClassLoader);
    }

    public ForkedGroovyScriptEngine(ResourceConnector rc) {
        this(null, CL_STUB, rc);
    }

    public ForkedGroovyScriptEngine(ResourceConnector rc, ClassLoader parentClassLoader) {
    ...
    public Class loadScriptByName(String scriptName) throws ResourceException, ScriptException {
        return loadScriptByName(scriptName,false);
    }
    public Class loadScriptByName(String scriptName,boolean forceRecompile) throws ResourceException, ScriptException {
        return loadScriptByName(scriptName,forceRecompile,null);
    }
    /**
     ...
     */
    public Class loadScriptByName(String scriptName,boolean forceRecompile,Class<? extends Annotation> desiredAnnotation) throws ResourceException, ScriptException {
        ...
        if (entry != null) clazz = entry.scriptClass;
        try {
            if (forceRecompile || (clazz != null && desiredAnnotation != null ? (clazz.isAnnotationPresent(desiredAnnotation)): false ) || isSourceNewer(entry)) {
                // if class has been loaded before and it had this annotation, force recompile
                try {
                    log.debug("recompiling path {}",path);
    ...
    protected boolean isSourceNewer(ScriptCacheEntry entry) throws ResourceException {
        ...
        for (String scriptName : entry.dependencies) {
            ...
            log.debug("minimumrecompilation interval: {}",config.getMinimumRecompilationInterval());
            log.debug("nextcheck={} now={}",nextSourceCheck,now);
            if (nextSourceCheck > now) continue;

            long lastMod = getLastModified(scriptName);
            log.debug("depentry.lastmodified={} lastmod={}",depEntry.lastModified,lastMod);
